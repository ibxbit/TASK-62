...
use super::models::{
    CallbackResponse, CallbackRow, CompensationJobResponse, CompensationJobRow,
...
pub async fn upload_import(
    ...
    let row = sqlx::query_as!(
        ...
        INSERT INTO payments.statement_imports
            (filename, file_hash, source, import_date, imported_by, raw_content_encrypted)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id, filename, file_hash, source, import_date,
                  status, total_records, processed_records,
                  error_count, imported_by, created_at, updated_at
        "#,
        body.filename,
        file_hash,
        body.source,
...
pub async fn process_import(
    ...
    // Load the import row
    let row = sqlx::query!(
        "SELECT status, raw_content_encrypted FROM payments.statement_imports WHERE id = $1",
        id,
    )
    .fetch_optional(&state.db)
    .await?
    ...
    if row.status == "completed" {
        return Err(AppError::BadRequest(
            "Import already processed".to_string(),
        ));
    }

    let encrypted = row.raw_content_encrypted.ok_or_else(|| {
        AppError::BadRequest("Import has no stored content".to_string())
    })?;
...
pub async fn create_refund(
    ...
    let existing = sqlx::query_as!(
        RefundRow,
        r#"
        SELECT id, transaction_id, idempotency_key, amount::double precision, reason,
    ...
    let row = sqlx::query_as!(
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id, transaction_id, idempotency_key, amount::double precision, reason,
                  status, requested_by, approved_by, processed_at,
                  created_at, updated_at
        "#,
        body.transaction_id,
...
pub async fn process_refund(
    ...
    let existing = sqlx::query!(
        "SELECT status, transaction_id, amount::double precision FROM payments.refunds WHERE id = $1",
        id,
    )
    .fetch_optional(&state.db)
    .await?
